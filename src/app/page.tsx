...
export default function Home() {
  ...
  useEffect(() => { 
    ...
    const today = getPolishDateString();
    
    let targetDate = (dateParam && /^\d{4}-\d{2}-\d{2}$/.test(dateParam)) 
      ? dateParam 
      : today;
  ...
  const calculateStreak = (wonDates: string[]) => {
    ...
    if (!datesSet.has(getPolishDateString(checkDate))) {
      ...
      if (!datesSet.has(getPolishDateString(checkDate))) return 0;
    }
  ...
  const initGame = async (targetDate: string) => {
    ...
    if (isTestMode) {
      ...
      if (challengeErr) {
        setDbError(`Błąd pobierania: ${challengeErr.message}`);
        setNotFound(true);
        return;
      }
  ...
  const handleGuess = () => {
    ...
    localStorage.setItem(`game_${gameDate}`, JSON.stringify({ 
      attempts: newAttempts, 
      state: isWin ? 'won' : isGameOver ? 'lost' : 'playing' 
    }));
  };
  ...
  if (notFound) {
    return (
      <div className="h-screen bg-black flex flex-col items-center justify-center font-mono text-zinc-400 p-6 text-center">
        ...
        <button onClick={backToToday} className="mt-8 px-6 py-3 bg-white text-black font-black rounded-xl uppercase tracking-widest active:scale-95 transition-transform">
          Wróć do dzisiaj
        </button>
      </div>
    );
  ...
  return (
    <main className="h-screen bg-black text-zinc-200 flex flex-col items-center p-4 overflow-hidden font-sans relative">
      ...
      <div className="absolute top-4 right-4 z-50 hidden sm:block">
        <button onClick={() => setIsTutorialOpen(true)} className="group flex items-center gap-2 px-3 py-2 border border-zinc-900 bg-zinc-950 rounded-xl hover:border-zinc-700 transition-all active:scale-95 shadow-2xl">
          <span className="text-lg group-hover:animate-bounce">❓</span>
      ...
      <nav className="w-full max-w-md flex justify-between items-center mb-2 pt-1 border-b border-zinc-900 pb-2">
        <div className="flex items-center gap-2 font-black tracking-tighter text-white">
          <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-black font-black text-sm">?</div>
        ...
        <div className="flex gap-2 items-center">
          <div className="flex items-center gap-1 bg-zinc-900 px-1 py-1 rounded-full border border-zinc-800">
            <button onClick={() => handleDateChange(getPrevDay(gameDate))} disabled={isEarliestDate} className={`w-8 h-8 flex items-center justify-center rounded-full ${isEarliestDate ? 'opacity-30' : 'hover:bg-zinc-700 text-white'}`}>
              <span className="text-sm font-black">{'<'}</span>
            </button>
            <div className="flex flex-col items-center justify-center min-w-[60px]">
              <span className="text-[8px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-0.5">{isToday ? 'Dzisiaj' : 'Archiwum'}</span>
              <span className={`text-[13px] font-black leading-none tracking-wider ${isToday ? 'text-white' : 'text-blue-400'}`}>{formatDisplayDate(gameDate)}</span>
            </div>
            <button onClick={() => handleDateChange(getNextDay(gameDate))} disabled={isToday} className={`w-8 h-8 flex items-center justify-center rounded-full ${isToday ? 'opacity-30' : 'hover:bg-zinc-700 text-white'}`}>
              <span className="text-sm font-black">{'>'}</span>
            </button>
          </div>
          <div className="flex items-center gap-3.5 bg-zinc-900 px-3 py-3 rounded-full border border-zinc-800">
            ...
            <span className="text-[16px] font-black text-white">{stats.streak}</span>
          </div>
        </div>
      </nav>
      ...
      <div className="w-full max-w-md flex flex-col flex-1 overflow-hidden space-y-2">
        <div className="bg-[#050505] border border-zinc-900 rounded-[2rem] flex flex-col shadow-2xl overflow-hidden relative">
          <div className="h-44 sm:h-48 w-full bg-zinc-950 flex items-center justify-center p-4 border-b border-zinc-900/50">
             <img src={item.image_url} alt="Target" className="max-h-full max-w-full object-contain pointer-events-none" />
          </div>

          <div className="p-4 space-y-3 text-center">
            <h1 className="text-[16px] font-black uppercase tracking-tight text-zinc-300 line-clamp-1">{item.title}</h1>
            <div className="grid grid-cols-3 gap-2">
              {hints.map((hint) => (
                <button key={hint.id} disabled={attempts.length < hint.threshold} onClick={() => setActiveHint(hint.id)}
                  className={`flex flex-col items-center py-1.5 rounded-xl border transition-all ${activeHint === hint.id ? 'border-blue-500 bg-blue-500/10' : attempts.length >= hint.threshold ? 'border-zinc-700 bg-zinc-900' : 'border-zinc-900 bg-black opacity-20'}`}>
            ...
            <div className="h-6 flex items-center justify-center border-y border-zinc-900/50">
               <div className="text-[13px] font-black text-blue-400 uppercase tracking-tighter">
            ...
            {gameState === 'playing' ? (
              <div className="space-y-4">
                <div className="flex justify-center items-end gap-2 px-2 w-full">
                  <input type="text" inputMode="decimal" value={inputValue} onChange={(e) => {
                    const val = e.target.value;
                    if (!/^[0-9.,]*$/.test(val)) return;
                    setInputValue(val);
                    const parsed = parseFloat(val.replace(',', '.'));
                    if (!isNaN(parsed)) setGuess(Math.min(Math.max(0, parsed), range.max * 2));
                  }} className="w-1/2 text-center text-4xl font-black text-white bg-transparent border-b-2 border-zinc-700 focus:border-white focus:outline-none pb-1" placeholder="0,00" />
                  <span className="text-sm font-bold text-zinc-600 pb-1.5 uppercase">pln</span>
                </div>
                <input type="range" min={range.min} max={range.max} step={step} value={guess} onChange={(e) => {
                ...
                <button onClick={handleGuess} className="w-full bg-white text-black font-black py-4 rounded-2xl text-sm uppercase tracking-[0.2em] active:scale-95 shadow-lg">
                  POTWIERDŹ {attempts.length + 1}/6
                </button>
              </div>
            ) : (
              <div className="p-4 rounded-2xl border border-zinc-800 bg-zinc-900/40 text-center animate-in zoom-in-95">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1 font-bold">Cena Prawidłowa</p>
                <div className={`text-3xl font-black ${gameState === 'won' ? 'text-green-400' : 'text-white'}`}>{formatPrice(item.price)} zł</div>
                {!isToday && (
                  <button onClick={backToToday} className="text-[10px] text-blue-400 mt-2 font-black uppercase tracking-widest italic border border-blue-500/30 px-3 py-1 rounded-full hover:bg-blue-900/30">
                    Wróć do dzisiaj
                  </button>
                )}
                {isToday && <p className="text-[10px] text-zinc-600 mt-2 font-black uppercase tracking-widest italic">Wróć jutro!</p>}
              </div>
            )}
          </div>
        </div>

        <div className="flex-1 overflow-y-auto px-1 space-y-2 custom-scrollbar pb-2">
          {attempts.map((att, i) => {
            ...
            let status = { text: "ZIMNO", icon: "❄️", style: "border-blue-900 bg-blue-950/40 text-blue-300" };
            
            if (isCorrect) status = { text: "IDEALNIE", icon: "🎯", style: "border-green-500 bg-green-500/10 text-green-400" };
            else if (diffPct < 2.5) status = { text: "PARZY!", icon: "🔥🔥🔥", style: "border-red-600/60 bg-red-600/15 text-red-300" };
            else if (diffPct < 5) status = { text: "GORĄCO", icon: "🔥🔥", style: "border-red-500/50 bg-red-500/10 text-red-400" };
            else if (diffPct < 15) status = { text: "CIEPŁO", icon: "🔥", style: "border-orange-500/40 bg-orange-500/10 text-orange-400" };

            return (
              <div key={i} className={`flex items-center justify-between border ${status.style} px-4 py-3 rounded-xl shadow-sm`}>
                <div className="w-1/3 flex items-baseline gap-1">
                   <span className="text-lg font-black text-white">{formatPrice(att)}</span>
                ...
                <div className="w-1/3 flex justify-center">
                  {!isCorrect && (
                    <div className="flex items-center gap-1.5 bg-black/40 px-2.5 py-1 rounded-md border border-zinc-800/60">
                      ...
                      <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{isTooHigh ? 'Za dużo' : 'Za mało'}</span>
                    </div>
                  )}
                </div>
      ...
      <footer className="w-full py-2 text-[9px] text-zinc-600 font-black flex flex-col gap-1 items-center border-t border-zinc-900 mt-auto bg-black">
        <div className="w-full flex justify-between items-center px-4">
          <span className="tracking-widest uppercase italic opacity-50">Osci5</span>
          <div className="flex gap-3">
            <button onClick={() => setIsContactOpen(true)} className="hover:text-white uppercase">[ Kontakt ]</button>
            <Link href="/politics" className="hover:text-white uppercase">[ Polityka ]</Link>
            <Link href="/terms" className="hover:text-white uppercase">[ Regulamin ]</Link>
          </div>
        </div>
      ...
      {isContactOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-6 w-full max-w-xs relative animate-in zoom-in-95">
            <button onClick={() => setIsContactOpen(false)} className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-white bg-zinc-900 rounded-full font-black">✕</button>
            <h2 className="text-lg font-black text-white uppercase tracking-widest mb-3 flex items-center gap-2"><span>✉️</span> Kontakt</h2>
            <div className="space-y-4 text-xs text-zinc-400">
              <p>Znalazłeś błąd lub masz pomysł na nową funkcję?</p>
              <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800">
                <p className="font-black uppercase tracking-widest text-[9px] text-zinc-500 mb-1">Napisz do nas:</p>
                <a href="mailto:kontakt.ilezato@gmail.com" className="text-blue-400 font-bold hover:underline text-sm">kontakt.ilezato@gmail.com</a>
              </div>
            </div>
          </div>
        </div>
